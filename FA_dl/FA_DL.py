...
def dl_page(user, section, page_i, page_p, sync, speed, force, quiet, db_only):
    ...
    for sub in page_p.findAll("figure"):
        ...
        ID = sub.get("id")[4:]
        fatl.log.verbose(
            f"DOWNLOAD PAGE -> user:{user} section:{section} page:{page_i} sub:{sub_i} ID:{ID}"
        )
        print(
            f"{user[0:5]: ^5} {page_i:0>3}/{sub_i:0>2} {section} | {ID:0>10} | ",
            end="",
            flush=True,
        )
...
def dl_gs(user, section, sync, speed, force, quiet, db_only):
    ...
    while True:
        ...
        if page_p == None:
            fatl.log.normal(
                f"DOWNLOAD {section_db[section]} -> user:{user} page:{page_i} empty"
            )
            if page_i == 1:
                fatl.log.normal(
                    f"DOWNLOAD {section_db[section]} -> user:{user} disabled section"
                )
        ...
        if page_p.find("figure") is None:
            fatl.log.normal(
                f"DOWNLOAD {section_db[section]} -> user:{user} page:{page_i} no subs"
            )
            if page_i == 1:
                if not quiet:
                    print(
                        f"{user[0:5]: ^5} 001/01 {section} | No submissions to download"
                    )
                return 1
            else:
                return 0
        ...
        page_ret = dl_page(
            user, section, page_i, page_p, sync, speed, force, quiet, db_only
        )
...
def update(users, sections, speed, force, index, db_only):
    ...
    users_db = favar.variables.db.execute(
        "SELECT user, folders FROM users ORDER BY user ASC"
    ).fetchall()
    t = int(time.time())
...
def download_main():
    ...
    while True:
        ...
        fatl.log.normal(
            f'DOWNLOAD MAIN -> users:"{users}" sections:"{sections}" options:"{options}"'
        )
        ...
        fatl.log.normal(
            f"DOWNLOAD MAIN -> upd:{upd} sync:{sync} force:{force} speed:{speed} index:{index} dbonly:{db_only} quit:{quit}"
        )

...
def usr_find_errors():
    ...
    while i < len(usrs):
        u = usrs[i]
        if any(
            u[0].lower() == usrs[j][0].lower() for j in range(0, len(usrs)) if j != i
        ):
            errs_repet.append(u)
        i += 1
...
def repair_subs(repair=True):
    ...
    while any(len(errs) for errs in (errs_id, errs_vl, errs_fl)):
        ...
        if len(errs_vl):
            ...
            for sub in errs_vl:
                ...
                if sub_check_values(sub):
                    favar.variables.db.execute(
                        f'UPDATE submissions SET title = "{sub[3]}" WHERE id = {ID}'
                    )
                    favar.variables.db.execute(
                        f'UPDATE submissions SET description = "{sub[5]}" WHERE id = {ID}'
                    )
                    favar.variables.db.execute(
                        f'UPDATE submissions SET tags = "{sub[6]}" WHERE id = {ID}'
                    )
                    favar.variables.db.commit()
                ...
                if not fadl.check_page("view/" + str(ID)):
                    print(" - Page Error", end="", flush=True)
                    favar.variables.db.execute(
                        f"UPDATE submissions SET server = 0 WHERE id = {ID}"
                    )
                    favar.variables.db.commit()
                    continue
            ...
            if errs_fl_mv:
                print(
                    f'{errs_fl_mv} new submission{"s"*bool(len(errs_fl_mv) != 1)} with files missing'
                )
...
def repair_subs_files(repair=True):
    ...
    print(
        "Analyzing submissions database for missing submission files ... ",
        end="",
        flush=True,
    )
    fatl.log.normal("REPAIR SUB FILES -> find submissions")
    subs = favar.variables.db.execute(
        "SELECT id, authorurl FROM submissions WHERE filename = 0 AND server = 1 ORDER BY AUTHORURL ASC, id ASC"
    )
    subs = [[si for si in s] for s in subs.fetchall()]
    print("Done")
    ...
    for i in range(0, len(subs)):
        ...
        print(
            f"{i+1:0{l}}/{L} - {subs[i][1][:10]: <10} {subs[i][0]:010}",
            end="",
            flush=True,
        )
        ...
        if dl_ret == 1:
            fatl.log.verbose(f"REPAIR SUB FILES -> ID:{subs[i][0]:010} file error")
            print(" - fail")
        elif dl_ret == 3:
            fatl.log.verbose(f"REPAIR SUB FILES -> ID:{subs[i][0]:010} not found")
            print(" - fail")
...
def repair_usrs(repair=True):
    ...
    print(
        f'Found {len(errs_names)} capitalized username{"s"*bool(len(errs_names) != 1)}'
    )
    print(
        f'Found {len(errs_namef)} incorrect full username{"s"*bool(len(errs_names) != 1)}'
    )
    print(f'Found {len(errs_foldr)} empty folder{"s"*bool(len(errs_foldr) != 1)}')
    print(
        f'Found {len(errs_fl_dl)} empty folder download{"s"*bool(len(errs_fl_dl) != 1)}'
    )
    ...
    while any(
        ...
        if len(errs_namef):
            ...
            for u in errs_namef:
                ...
                u_db = favar.variables.db.execute(
                    f'SELECT author FROM submissions WHERE authorurl = "{u[0]}"'
                ).fetchall()
                if len(u_db):
                    u_db = u_db[0][0]
                    print(f"- db: {u_db}")
...
def repair_info(repair=True):
    ...
    print("Done")
    print(
        f'Found {len(errs_reps)} repeated entr{"ies"*bool(len(errs_reps) != 1)}{"y"*bool(len(errs_reps) == 1)}'
    )
    ...
    if any(
        err
        for err in (errs_reps, errs_vers, errs_name, errs_nums, errs_timu, errs_timd)
    ):
        if len(errs_reps):
            print()
            ...
            for err in errs_reps:
                favar.variables.db.execute(
                    f'DELETE FROM infos WHERE field = "{err[0]}"'
                )
            print("Done")
        ...
        if errs_nums:
            ...
            favar.variables.db.execute(
                f'INSERT INTO INFOS (FIELD, VALUE) VALUES ("USRN", {table_n(db, "USERS")})'
            )
            favar.variables.db.execute(
                f'INSERT INTO INFOS (FIELD, VALUE) VALUES ("USRN", {table_n(db, "SUBMISSIONS")})'
            )
            favar.variables.db.commit()
            print("Done")

...
def temp_new():
    ...
    db_old.execute(
        "CREATE TABLE IF NOT EXISTS db_new.SUBMISSIONS AS SELECT * FROM main.SUBMISSIONS"
    )
    db_old.commit()
    print("Done")
    ...
    for u in usrs_new:
        ...
        exists = db_new.execute(
            f'SELECT EXISTS(SELECT name FROM users WHERE name = "{u[0]}" LIMIT 1);'
        )
...
def temp_import():
    ...
    usrs_new = db_new.execute(
        "SELECT user, userfull FROM users ORDER BY name ASC"
    ).fetchall()
...
def db_upgrade_v2v2_3():
    ...
    if not Session:
        ...
        print(
            "If all necessary data cannot be found in the database the update will be interrupted"
        )
    print()
    ...
    for u in usrs_new:
        ...
        if len(u_db):
            ...
            try:
                ...
                with open("FA.v2v2_3.error.txt", "w") as f:
                    ...
                    for e in err:
                        f.write(repr(e) + "\n")
                print(
                    "Informations on the error have been written to FA.v2v2_3.error.txt"
                )
                print("Update interrupted, it may be resumed later")
                if len(missing) > 0:
                    print(
                        f"Found {len(missing)} user/s no longer present on the website"
                    )
                if missing_db > 0:
            ...
            if u_fa:
                u_fa = (
                    u_fa.lstrip("Userpage of ")
                    .rstrip(" -- Fur Affinity [dot] net")
                    .strip()
                )
            else:
                u_fa = u[0]
